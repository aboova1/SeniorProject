"""
Backtest Proportional Threshold Strategy
=========================================

Strategy:
1. Only invest in stocks predicted to go up by MORE than a threshold (e.g., 10%)
2. Investment amount is PROPORTIONAL to predicted return magnitude
3. Larger predicted gains → larger investment
4. Rebalance quarterly with fixed total capital pool

This combines threshold filtering with proportional weighting for optimal allocation.
"""
...
logger = logging.getLogger("backtest_proportional_threshold")
...
# ============================= Backtesting ===================================
def backtest_proportional_threshold_strategy(
    predictions_df: pd.DataFrame,
    threshold_pct: float = 0.10,
    total_capital_per_quarter: float = 10000.0
) -> Dict:
    """
    Backtest proportional investment with threshold filtering.

    Strategy:
    - Filter to stocks predicted to gain > threshold
    - Allocate capital proportionally based on predicted return magnitude
    - Stocks with higher predicted returns get more capital
    ...
    """
    logger.info("="*70)
    logger.info(f"BACKTESTING PROPORTIONAL THRESHOLD STRATEGY (>{threshold_pct*100:.0f}%)")
    logger.info("="*70)
    ...
    for quarter_date, quarter_group in predictions_df.groupby('period_start_date'):
        ...
        # Calculate investment weights based on predicted return magnitude
...
# ============================= Main ==========================================
def main():
    parser = argparse.ArgumentParser(description="Backtest proportional investment with threshold filtering")
    ...
    parser.add_argument("--output", type=str, default="backtest_proportional_threshold_results.json",
    ...
    # Run backtest
    backtest_results = backtest_proportional_threshold_strategy(
        predictions_df,
    ...
    print("\n" + "="*70)
    print(f"FINAL BACKTEST SUMMARY - PROPORTIONAL THRESHOLD STRATEGY (>{summary['threshold_pct']:.0f}%)")
    print("="*70)
