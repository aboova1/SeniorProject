"""
Backtest Threshold Sweep - Test Multiple Thresholds
====================================================

Strategy:
Run the threshold strategy for multiple threshold values (0.01 to 0.30 in 0.01 increments)
to find the optimal threshold that maximizes annualized ROI.

This helps determine the best confidence level for investment decisions.
"""
...
logger = logging.getLogger("backtest_threshold_sweep")
...
def backtest_single_threshold(
    ...
    """Backtest a single threshold value (silent mode - no logging).

    Args:
        predictions_df: DataFrame with predictions
        threshold_pct: Minimum predicted return threshold
        investment_per_stock: Fixed dollar amount to invest per stock (default: $1000)
    """

    # Sort by period_start_date (actual transcript availability)
...
# ============================= Main ==========================================
def main():
    parser = argparse.ArgumentParser(description="Sweep threshold values to find optimal threshold")
    ...
    parser.add_argument("--output", type=str, default="backtest_threshold_sweep_results.json",
    ...
    for threshold in thresholds:
        ...
        result = backtest_single_threshold(predictions_df, threshold, args.investment_per_stock)
