...
logger = logging.getLogger("train_lstm_quant_only")
...
class LSTMTrainer:
    ...
    def load_and_prepare_data(self):
        ...
        # Feature cols: exclude base columns, price targets, AND TEXT FEATURES
        all_cols = train_df.columns.tolist()
        exclude_cols = set(BASE_EXCLUDE_COLS)

        # QUANTITATIVE ONLY: Filter out all text embeddings (txt_*)
        self.feature_cols = [c for c in all_cols if c not in exclude_cols and not c.startswith("txt_")]

        logger.info(f"Using {len(self.feature_cols)} QUANTITATIVE features (text embeddings excluded)")
    ...
    def train(self, num_epochs=80, batch_size=64, lr=2e-3, weight_decay=1e-5, patience=12):
        logger.info("="*60); logger.info("STARTING TRAINING (log-space, residual to Q7, QUANT ONLY)"); logger.info("="*60)
        ...
        for ep in range(num_epochs):
            ...
            if vl_loss < best:
                ...
                }, self.model_dir / "best_model_quant_only.pt")
        ...
        with open(self.results_dir / "training_history_quant_only.json", "w") as f:
    ...
    def evaluate_test_set(self):
        ...
        ckpt = torch.load(self.model_dir / "best_model_quant_only.pt", map_location=self.device, weights_only=False)
        ...
        logger.info("="*60); logger.info("TEST SET RESULTS (raw units, QUANT ONLY)"); logger.info("="*60)
        ...
        with open(self.results_dir / "test_results_quant_only.json", "w") as f:
            json.dump({
...
def parse_args():
    p = argparse.ArgumentParser(description="Train LSTM with QUANTITATIVE features only (no text embeddings)")
...
def main():
    ...
    print("[SUCCESS] Training complete (QUANTITATIVE ONLY)!")
    print(f"[TEST RMSE] {test_metrics['rmse']:.4f}")
    print(f"[TEST MAPE] {test_metrics['mape']:.4f}")
    print(f"[TEST IC]   {test_metrics['ic']:.4f}")
    print(f"[MODEL] Saved to {Path('models') / 'best_model_quant_only.pt'}")
