...
logger = logging.getLogger("train_lstm_no_price")
...
PRICE_DERIVED_COLS = [
    ...
    "enterprise_value",  # includes market_cap
    "pe_ratio",          # = price / earnings
    "earnings_yield",    # = earnings / price
    "momentum_12m"       # price momentum
]

# ----------------------------- Dataset ---------------------------------
class SequenceDatasetNoPrice(Dataset):
    """
    Returns tuples: (x_7xF, y8_log, y7_raw, y8_raw)
      - x_7xF: first 7 rows (Q1..Q7) of features (scaled) - NO PRICE FEATURES
      - y8_log: log1p(target_price at Q8)
      - y7_raw: raw current_price at Q7 (for directional eval only)
    ...
    def __init__(self, sequences_df: pd.DataFrame, feature_cols: List[str]):
        ...
        for sid in self.sequence_ids:
            ...
            # Features: Q1..Q7 only (NO PRICE FEATURES)
            ...
            self.samples.append(
                (
                    ...
                    torch.tensor([float(y7)], dtype=torch.float32),  # y7_raw for eval
                    torch.tensor([float(y8)], dtype=torch.float32),
                )
            )
            valid += 1

        logger.info(f"Loaded {valid} valid sequences (7q -> Q8 target_price, NO PRICE FEATURES)")
...
class UniLSTMAttnDirect(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int = 192, num_layers: int = 2, dropout: float = 0.25, use_attn: bool = True):
        ...
        self.fc2 = nn.Linear(128, 1)        # predicts y8_log DIRECTLY

    def forward(self, x: torch.Tensor):
        # x: (B, 7, F) - NO y7_log input!
        ...
        if self.use_attn:
            ...
            rep = last_h                            # (B, H)
        rep = self.ln(rep)
        # NO concatenation with y7_log!
        z = self.fc1(rep)
        z = self.act(z)
        z = self.drop(z)
        y8_log_hat = self.fc2(z)  # DIRECT prediction (not residual!)
        return y8_log_hat

# ---------------------------- Trainer ----------------------------------
class LSTMTrainerNoPrice:
    def __init__(self,
                 train_path: str,
                 val_path: str,
                 test_path: str,
                 model_dir: str = "models_no_price",
                 results_dir: str = "results_no_price",
                 use_attn: bool = True,
                 loss_name: str = "huber"):
    ...
    def _analyze_features(self) -> dict:
        ft = {
            "technical": sum(1 for c in self.feature_cols if any(x in c for x in ["vol", "beta"])),  # NO momentum!
    ...
    def load_and_prepare_data(self):
        ...
        # Feature cols: exclude base columns AND price-derived features
        all_cols = train_df.columns.tolist()
        exclude_cols = set(BASE_EXCLUDE_COLS) | set(PRICE_DERIVED_COLS)
        self.feature_cols = [c for c in all_cols if c not in exclude_cols]

        logger.info(f"Using {len(self.feature_cols)} features (EXCLUDING price-derived: {PRICE_DERIVED_COLS})")
        ...
        # Build datasets (NO PRICE)
        self.train_ds = SequenceDatasetNoPrice(train_df, self.feature_cols)
        self.val_ds   = SequenceDatasetNoPrice(val_df,   self.feature_cols)
        self.test_ds  = SequenceDatasetNoPrice(test_df,  self.feature_cols)
    ...
    def create_model(self, hidden_size: int = 192, num_layers: int = 2, dropout: float = 0.25):
        ...
        logger.info(f"Architecture: {num_layers}-layer unidirectional LSTM ({hidden_size} units)"
                    f" -> {'Attn+' if self.use_attn else ''}Direct-Prediction (NO Q7 price) -> y8_log")
        return self.model
    ...
    def _epoch(self, loader, optimizer=None, criterion=None):
        ...
        for batch in loader:
            X, y8_log, y7_raw, y8_raw = batch  # shapes: (B,7,F), (B,1), (B,1), (B,1)
            X = X.to(self.device); y8_log = y8_log.to(self.device); y8_raw = y8_raw.to(self.device)
            if train_mode:
                optimizer.zero_grad()
            y8_log_hat = self.model(X)  # NO y7_log passed!
    ...
    def train(self, num_epochs=80, batch_size=64, lr=2e-3, weight_decay=1e-5, patience=12):
        ...
        with open(self.results_dir / "training_history_no_price.json", "w") as f:
    ...
    def evaluate_test_set(self):
        ...
        ckpt = torch.load(self.model_dir / "best_model_no_price.pt", map_location=self.device, weights_only=False)
        ...
        for batch in test_ld:
            X, y8_log, y7_raw, y8_raw = batch
            X = X.to(self.device); y8_log = y8_log.to(self.device); y8_raw = y8_raw.to(self.device); y7_raw = y7_raw.to(self.device)

            with torch.no_grad():
                y8_log_hat = self.model(X)  # NO y7_log!
                loss = criterion(y8_log_hat, y8_log)
                total_loss += float(loss.item())

                pred_raw = torch.expm1(y8_log_hat).cpu().numpy().flatten()
                targ_raw = y8_raw.cpu().numpy().flatten()
                y7_raw_np = y7_raw.cpu().numpy().flatten()

                pred_raw_all.extend(pred_raw)
                targ_raw_all.extend(targ_raw)
                y7_raw_all.extend(y7_raw_np)
...
def parse_args():
    p = argparse.ArgumentParser(description="Train LSTM without price features (7q -> Q8 target)")
...
def main():
    ...
    print(f"[MODEL] Saved to {Path('models_no_price') / 'best_model_no_price.pt'}")
